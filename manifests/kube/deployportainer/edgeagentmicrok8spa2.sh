...
main() {
    ...
    curl -L  https://raw.githubusercontent.com/jamest-portainer/testing/main/manifests/kube/deployportainer/pa2.yaml -o pa2.yaml || errorAndExit "Unable to download agent manifest"
    
    info "Creating Portainer namespace..."
    microk8s kubectl create namespace portainer2
    
    info "Creating agent configuration..."
    configmapCmd="microk8s kubectl create configmap -n portainer2 portainer-agent-edge2"
    ...
    microk8s kubectl create secret generic portainer-agent-edge-key2 "--from-literal=edge.key=$EDGE_KEY" -n portainer2
    
    info "Deploying agent..."
    microk8s kubectl apply -f pa2.yaml || errorAndExit "Unable to deploy agent manifest"
